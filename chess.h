...
typedef struct chess_mode Mode;
...
#define WKING_START_POS	60
#define BKING_START_POS	4

#define BIT(n) 			(1UL << (n))
#define SET_BIT(x, n)	((x) |= BIT((n)))
#define RESET_BIT(x, n) ((x) &= ~BIT(n))
#define IS_SET(x, n) 	((x) & BIT((n)))
#define BOARD(x) 		((player[x].q) | (player[x].k) | (player[x].r) | (player[x].n) | (player[x].p) | (player[x].b))
#define FULL_BOARD		((BOARD(WHITE) | (BOARD(BLACK))))
#define OCCUPIED(n)		(FULL_BOARD & (BIT(n)))
